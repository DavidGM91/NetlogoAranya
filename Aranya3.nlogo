...
aranyes-own[
  PresaAct ; Hacer un lock sobre la presa podria mejorar la eficiencia cuando hay multiples a la misma distancia
  Teranyna_x
  Teranyna_y ; Posició de la meva teranyna, per veure si ja la he fet i estrategia
  Menjant
]
...
to eat-aranya
  ...
  [
    ...
    [
      let myvida Vida
      if (count other Aranyes-here with [Vida < myvida] > 0) and (Inanició > 300)
      and not( (count other Aranyes-here with [Vida >= 1000] > 0) and ( Fertilitat > 799 ))
      [
        let presa one-of other Aranyes-here with [Vida < myvida]
...
to moure-aranya
  let myvida Vida
  let hambrienta (Inanició > 300)
  let preses sort-on [distance myself] other turtles with[breed = Papallones or (breed = Granotes and Vida < 250) or (hambrienta and breed = Aranyes and Vida < myvida)] in-radius 15
  ;TODO: Evitar presas en grupos de 5
  let predadors sort-on [distance myself] turtles with[breed = Talps or (breed = Granotes and Vida > 250)] in-radius 15
  let carnivs sort-on [distance myself] other turtles with[breed = Carnivores] in-radius 3
  ifelse empty? carnivs
  [
    ifelse empty? preses
    [
      ;TODO: Mejorar busqueda cuando no se ve una presa
      rt random-float 90 - random-float 90
    ]
    [
      set heading towards first preses
    ]
  ]
  [
    set heading towards first carnivs
    rt 90
  ]
  fd 2
end
...
to crea_teranyina
  ...
  [
    set Teranyna_x xcor
    set Teranyna_y ycor
    ask patches in-radius 2.5 [set  teranyina false]
...
to morir-Aranyes [qui]
  let murderBreed [breed] of qui
  ifelse (count Aranyes-here > 4) [
    ;TODO: Penalitzar especie murderBreed
  ]
  [
    ifelse( not( (murderBreed = Granotes) or (murderBreed = Talps) or (murderBreed = Carnivores) or (murderBreed = Aranyes) ) )
    [
      ;TODO: Penalitzar especie murderBreed
    ]
    [
      ifelse( (murderBreed = Granotes) and ([Vida] of qui < 250) )
      [
        ;TODO: Penalitzar Granotes
      ]
      [
        set color red
        die
